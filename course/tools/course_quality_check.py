...
def check_manifest() -> None:
    ...
    required_preflight_terms = {
        ...
        "install_script": "course/install-agents.sh",
    }
    ...
    if manifest.get("provider_setup_playbook") != "course/docs/account-setup.md":
        errors.append(f"{rel(path)} must point to account setup playbook")

    if manifest.get("teaching_protocol") != "course/docs/teaching-protocol.md":
        errors.append(f"{rel(path)} must point to teaching protocol")

    if manifest.get("course_flow") != "course/days/single.md":
